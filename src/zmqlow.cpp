...
int sendMultipartMessage(void* socket, int flags, const T& part)
{
    ...
    if (BOOST_UNLIKELY(rc != 0)) {
        LOG_ERROR(log::Arg{"zmq_msg_init_size", log::ec_t{zmq_errno()}});
        return -1;
    }
    ...
    if (BOOST_UNLIKELY(rc == -1)) {
        if (errno != EAGAIN) {
            LOG_ERROR(log::Arg{"zmq_send", log::ec_t{zmq_errno()}});
        }
...
int recvMultipartMessage(void* socket, int flags, T* part)
{
    ...
    if (BOOST_UNLIKELY(rc == -1)) {
        if (errno != EAGAIN) {
            LOG_ERROR(log::Arg{"zmq_msg_recv", log::ec_t{zmq_errno()}});
        }
    } else {
...
void* initContext()
{
    ...
    if (!context) {
        LOG_ERROR(log::Arg{"zmq_ctx_new", log::ec_t{zmq_errno()}});
    }
...
bool deleteContext(void* context)
{
    ...
    if (rc == -1) {
        LOG_ERROR(log::Arg{"zmq_ctx_term", log::ec_t{zmq_errno()}});
    }
...
void* createSocket(void* context, int type)
{
    ...
    if (!socket) {
        LOG_ERROR(log::Arg{"zmq_socket", log::ec_t{zmq_errno()}});
    }
...
bool closeSocket(void* socket)
{
    ...
    if (rc == -1) {
        LOG_ERROR(log::Arg{"zmq_close", log::ec_t{zmq_errno()}});
    }
...
bool connectSocket(void* socket, const char* endpoint)
{
    ...
    if (rc == -1) {
        LOG_ERROR(log::Arg{"zmq_connect", log::ec_t{zmq_errno()}});
    }
...
bool bindSocket(void* socket, const char* endpoint, int timeout)
{
    ...
    if (rc == -1) {
        LOG_ERROR(log::Arg{"zmq_bind", log::ec_t{zmq_errno()}});
    }
...
bool pollSocket(zmq_pollitem_t* items, int nitems, long msecs)
{
    ...
    if (rc == -1) {
        LOG_ERROR(log::Arg{"zmq_poll", log::ec_t{zmq_errno()}});
    }
...
bool setSocketOption(void* socket, int option, int value)
{
    ...
    if (rc == -1) {
        LOG_ERROR(log::Arg{"zmq_setsockopt", log::ec_t{zmq_errno()}});
    }
...
int socketOption(void* socket, int option, int defaultValue, bool* ok)
{
    ...
    *ok = rc != -1;
    ...
    if (!*ok) {
        LOG_ERROR(log::Arg{"zmq_getsockopt", log::ec_t{zmq_errno()}});
    }
